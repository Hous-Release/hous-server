...
public class NotificationRetrieveServiceTest {
	...
	public void get_notifications_info_by_scroll_pagination_success() {
		...
		// when
		NotificationsInfoResponse response1 = notificationRetrieveService.getNotificationsInfo(1, Long.MAX_VALUE,
			userId);
		NotificationsInfoResponse response2 = notificationRetrieveService.getNotificationsInfo(1,
			response1.getNextCursor(), userId);
		NotificationsInfoResponse response3 = notificationRetrieveService.getNotificationsInfo(1,
			response2.getNextCursor(), userId);

		NotificationsInfoResponse response4 = notificationRetrieveService.getNotificationsInfo(1, Long.MAX_VALUE,
			userId);
		NotificationsInfoResponse response5 = notificationRetrieveService.getNotificationsInfo(1,
			response4.getNextCursor(), userId);
		NotificationsInfoResponse response6 = notificationRetrieveService.getNotificationsInfo(1,
			response5.getNextCursor(), userId);
