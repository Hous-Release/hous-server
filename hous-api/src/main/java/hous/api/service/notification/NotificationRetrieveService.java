...
public class NotificationRetrieveService {
	...
	public NotificationsInfoResponse getNotificationsInfo(int size, Long lastNotificationId, Long userId) {
		...
		List<Notification> notifications = notificationRepository.findNotificationsByOnboardingAndCursor(me,
			lastNotificationId, size + 1);
		ScrollPaginationCollection<Notification> notificationsCursor = ScrollPaginationCollection.of(notifications,
			size);
		NotificationsInfoResponse response = NotificationsInfoResponse.of(notificationsCursor,
			notificationRepository.countAllByOnboarding(me));
